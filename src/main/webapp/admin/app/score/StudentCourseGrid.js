Ext.define('bit.score.StudentCourseGrid', {
    ...
    initComponent: function() {
     ...
     this.store = Ext.create("Ext.data.Store",{
			...
			fields : ["id", "course.id", "course.name", "course.no", "course.teacher.user.name","course.teacher.user.number", "student.user.name", 
				"student.user.number", "student.id", "createTime", "updateTime"],
			scope: this,
			pageSize : 20,
			proxy : {
				type: 'ajax',
	...
	onAddDD : function() {
		var me = this;
		var edit = Ext.create('bit.score.StudentCourseWindow',{
			title:'添加',
			storeId : me.storeId
		});
		edit.show();
	...
	onDeleteDD : function() {
		...
		var store = Ext.data.StoreManager.lookup(me.storeId);
		 Ext.MessageBox.confirm("确认", "删除课程为[" + record.get("course.name") + "]学生为[" + record.get("student.user.name") + "]的安排？", function(btn){
			  if(btn == "yes"){
