Ext.define('bit.system.StudentGrid', {
    ...
    initComponent: function() {
     ...
     this.store = Ext.create("Ext.data.Store",{
			...
			fields : ["id", "user.id", "user.username", "user.name", "user.sex", "user.number", "user.phone", "user.password",
				"user.idNo", "user.email","user.homeAddress","college", "department","dormitory","grade",
	...
	onAddDD : function() {
		var me = this;
		var edit = Ext.create('bit.system.StudentWindow',{
			title:'添加',
			storeId : me.storeId
		});
		edit.show();
	...
	onDeleteDD : function() {
		...
		var store = Ext.data.StoreManager.lookup(me.storeId);
		 Ext.MessageBox.confirm("确认", "删除名字为[" + record.get("user.name") + "]的学生？", function(btn){
			  if(btn == "yes"){
				  Ext.getBody().mask("正在删除数据…");
				  //提交删除
				  Ext.Ajax.request({
					  url: "system/student/del?id=" + record.get("id"),
					  disableCaching: true,
					  method: "get",
					  //删除成功
